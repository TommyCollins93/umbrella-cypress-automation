...
describe("creating user account for iso 27001 and 27701 transfer - 4th step", function(){

    it("creating user account for iso 27001 and 27701 transfer - 4th step", function(){
        ...
        cy.get('input[formcontrolname="firstName"]').type("FirstName27and277tran4step");
        cy.get('input[formcontrolname="lastName"]').type("LastName");
        cy.get('input[formcontrolname="jobTitle"]').type("JobTitle");
        cy.get('input[formcontrolname="email"]').type("autouser27and277tran-step4@qq.qq");
        cy.get('input[formcontrolname="phone"]').type("(512) 512 -  5122");
        cy.get('input[formcontrolname="companyName"]').type("Company27and277tran4step");
        ...
        cy.get("[id='desired-next-audit-date']").each(($datenext)=>{
            cy.get($datenext).click();
            for(let n=0 ; n<2 ; n++){
        ...
        cy.get("#transfer-reason-iso_27701-certificate").type("Some another reason");
        ...
        cy.get("#isCurrentCertificationBodyAccredited-iso_27701-certificate").find(".text").contains("No").click();
        ...
        cy.get("[id='last-audit-performed-date']").each(($datelast)=>{
            cy.get($datelast).click();
            for(let n=0 ; n<2 ; n++){
                cy.get(".cdk-overlay-pane").find("g[data-name='chevron-left']").click();
            }
            cy.get(".cdk-overlay-pane").find(".day-cell").contains("15").click();
        })
        cy.get("[id='expiration-date']").each(($dateexp)=>{
            cy.get($dateexp).click();
            for(let n=0 ; n<2 ; n++){
                cy.get(".cdk-overlay-pane").find("g[data-name='chevron-right']").click();
            }
        ...
        cy.get("#text-question-0ISO_27701-iso_27701").type("Some kind of description.");
        cy.get("#text-question-1ISO_27701-iso_27701").type("Lots of controls.");
