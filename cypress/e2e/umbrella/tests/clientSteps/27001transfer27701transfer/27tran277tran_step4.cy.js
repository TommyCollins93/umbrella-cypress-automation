...
describe("27001 and 27701 transfer - 4th step testcases", function(){

    beforeEach(function(){
        cy.visit("https://stage-iso14001.myaudit.net/system/login");
        cy.get("#email").type("autouser27and277tran-step4@qq.qq");
    ...
    it("basic validations/contact banner/stepper/downloading PDF", function(){
        ...
        cy.readFile('cypress\\downloads\\Estimate-Company27and277tran4step.pdf').should('exist');
    ...
    it("right hand side - title, files and user data", function(){
        ...
        cy.get("#terms-and-conditions-first-name").should("have.value", "FirstName27and277tran4step");
    ...
    describe("billing info cases", function(){
        ...
        it("billing info section", function(){
            ...
            cy.wait(5000);
                ...
                cy.get("#user-info").find("#user-full-name").should("have.text", "FirstName27and277tran4step LastName");
                cy.get("#user-info").find("#user-email").should("have.text", "autouser27and277tran-step4@qq.qq");
                //check basic info accordion - first/last name and email change shouldn't change basic info data
                cy.get(".order-card").find(".company-name").should("have.text", "Company27and277tran4step");
                cy.get("#order-details").find("#contact-person").should("have.text", "FirstName27and277tran4step LastName");
                cy.get("#order-details").find("#email").should("have.text", "autouser27and277tran-step4@qq.qq");
            ...
            cy.get("input[formcontrolname='email']").clear().type("autouser27and277tran-step4@qq.qq");
    ...
    describe("edit functionality", function(){
        ...
        it("basic info - edit", function(){
            ...
            cy.get("basic-order-edit").find("current-cb-questions").each(($cb)=>{
                cy.get($cb).should("exist");
            });
            cy.get("basic-order-edit").find("company-general-info").each(($genInfo)=>{
                cy.get($genInfo).should("exist");
            });
            cy.get("basic-order-edit").find("estimate-questions").each(($estQuestions)=>{
                cy.get($estQuestions).should("exist");
            });
            ...
            cy.get("#terms-and-conditions-first-name").should("have.value", "FirstName27and277tran4step edit");
            ...
            cy.get(".order-card").find(".company-name").should("have.text", "Company27and277tran4step edit");
            ...
            cy.get("order-details").find("#change-button").click();
                ...
                cy.get("#current-certification-last-audit-event-radio-group").each(($lastAuditEvent)=>{
                    cy.get($lastAuditEvent).find(".text").contains("Recertification").click();
                });
                ...
                cy.get(".question-options").each(($question)=>{
                    cy.get($question).find(".question-option").eq(1).click();
                });
            //
            cy.get("basic-order-edit").find("#change-button").click();
            cy.wait(5000);
            ...
            it("additional questions - edit", function(){
                ...
                //section visibility
                cy.get(".additional-questions-card").find("#toggle-order-details").click();
                cy.get(".additional-questions-card").find("transfer-additional-questions-details").each(($trDetails)=>{
                    cy.get($trDetails).should("exist");
                });
                cy.get(".additional-questions-card").find("base-additional-questions-details").each(($baseDetails)=>{
                    cy.get($baseDetails).should("exist");
                });
                ...
                cy.get(".additional-questions-card").find("#change-button").click();
                    ...
                    cy.get("#decline-offer-button").should("have.attr", "disabled");
                    cy.get(".submit-button").should("have.attr", "disabled");
                cy.get(".audit-date-row").each(($nextAudit)=>{
                    cy.get($nextAudit).find(".clear-icon").click();
                });
                cy.get("#transfer-reason-iso_27001-certificate").type(" and another reason");
                cy.get("#transfer-reason-iso_27701-certificate").type(" and another reason");
                cy.get("#isCurrentCertificationBodyAccredited-iso_27001-certificate").find(".text").contains("Yes").click();
                cy.get("#hasOpenNonConformities-iso_27001-certificate").find(".text").contains("No").click();
                cy.get("#isCurrentCertificationBodyAccredited-iso_27701-certificate").find(".text").contains("Yes").click();
                cy.get("#hasOpenNonConformities-iso_27701-certificate").find(".text").contains("No").click();
                ...
                cy.get("[id='expiration-date']").each(($expDate)=>{
                    cy.get($expDate).click();
                    cy.get(".cdk-overlay-pane").find(".day-cell").contains("20").click();
                });
                //======
                ...
                cy.get("#binary-question-description-0ISO_27001-iso_27001").type("Some secutity issues.");
                cy.get("#binary-question-radio-1ISO_27001-iso_27001").find(".text").contains("No").click();
                cy.get("#binary-question-radio-2ISO_27001-iso_27001").find(".text").contains("Yes").click();
                cy.get("#binary-question-description-2ISO_27001-iso_27001").type("Some risks.");
                cy.get("#binary-question-radio-3ISO_27001-iso_27001").find(".text").contains("No").click();
                cy.get("#binary-question-radio-4ISO_27001-iso_27001").find(".text").contains("Yes").click();
                cy.get("#binary-question-description-4ISO_27001-iso_27001").type("Receiving services.");
                //
                cy.get("#binary-question-radio-0ISO_27701-iso_27701").find(".text").contains("No").click();
                cy.get("#text-question-0ISO_27701-iso_27701").type(" Another edit.");
                cy.get("#text-question-1ISO_27701-iso_27701").type(" And more.");
                //
