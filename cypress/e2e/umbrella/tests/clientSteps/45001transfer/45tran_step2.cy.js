...
describe("45001 transfer - 2nd step testcases", function(){

    beforeEach(function(){
        cy.visit("https://stage-iso14001.myaudit.net/system/login");
        cy.get("#email").type("autouser45tran-step2@qq.qq");
    ...
    it("basic validations", function(){
        ...
        cy.get("input[formcontrolname='customCbNameISO_45001']").type("a");
        cy.get(".custom-cb").find(".validation-message").should("have.text", "The answer is too short");
        cy.get("input[formcontrolname='customCbNameISO_45001']").clear().type("qwertyuiu ".repeat(10)+"a");
        cy.get(".custom-cb").find(".validation-message").should("have.text", "The answer is too long");
        //company description
        cy.get("#company-description_-iso_45001").type("aaaaaaaaa");
        ...
        // cy.get("#company-description_-iso_45001").clear().type("qwertyuiu ".repeat(500)+"a");
        ...
        cy.get("#employees-within-scope-iso_45001").clear().type("2025");
        ...
        cy.get("#part-time-employees-included-iso_45001-true").click();
        cy.get(".submit-button").click();
        cy.get(".text-area-wrapper").eq(2).find(".validation-message").should("have.text", "Required");
        //zabiera za dużo czasu, czeka na dokończenie animacji
        // cy.get("#employees-count-description-iso_45001").type("qwertyuiu ".repeat(500)+"a");
        // cy.get(".text-area-wrapper").eq(2).find(".validation-message").should("have.text", "The answer is too long");
        //number of shifts
        cy.get("#shifts_iso_45001").find(".status-primary").contains("More").click();
        cy.get(".submit-button").click();
        ...
        cy.get(".submit-button").click();
        cy.get(".offer-request-card").eq(1).find("#attached-question-translator_required-iso_45001-specify_language").should("exist");
        cy.get(".offer-request-card").eq(1).find(".validation-message").eq(0).should("have.text", "Required");
        cy.get("#attached-question-translator_required-iso_45001-specify_language").type("a");
        cy.get(".offer-request-card").eq(1).find(".validation-message").eq(0).should("have.text", "The answer is too short");
        cy.get("#attached-question-translator_required-iso_45001-specify_language").clear().type("qwertyuiu ".repeat(10)+"a");
    ...
    it("choosing DEKRA in cb section/info icons/accordion behaviours)", function(){
        ...
        //choosing DEKRA
        cy.get("#has-dekra-iso_45001-certificate-radio-group").find(".status-primary").contains("DEKRA").click();
        ...
        cy.get(".ng-dropdown-panel").find(".ng-option").contains("Not on the list").click();
        cy.get("input[formcontrolname='customCbNameISO_45001']").type("dEkRa");
        cy.get(".redirection-message").should("have.text"," If you need a renewal, please get in touch with our sales department sales.us@dekra.com");
        cy.get(".submit-button").should("have.class", "btn-disabled");
        cy.get("button[disabled]").should("exist");
        cy.get("input[formcontrolname='customCbNameISO_45001']").clear();
    ...
    it("certification scopes/number of shifts/stepper", function(){
        ...
        cy.get(".ng-dropdown-panel-items").find(".ng-option").should("have.length", 11);
        cy.get("#company-scopes").type("ra");
        cy.get(".ng-dropdown-panel-items").find(".ng-option").should("have.length", 2);
        cy.get(".ng-dropdown-panel-items").find(".ng-option").eq(0).should("have.text", "03 - Food product, beverages and tobacco");
        cy.get(".ng-dropdown-panel-items").find(".ng-option").eq(1).should("have.text", "31 - Transport, Storage and Communications");
        ...
        cy.get(".scope-question").should("have.length", 1);
        cy.get(".scope-question").each(($scope), function(){
            get($scope).find("nb-radio-group").contains("Yes").click();
        });
        //number of shifts
        cy.get("#shifts_iso_45001").find(".status-primary").contains("More").click();
